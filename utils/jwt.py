...
class JsonWebToken:
    ...
    def encode(self, sub: str) -> tuple[str, str]:
        payload = {"sub": sub, "exp": datetime.now() + timedelta(seconds=self._expiry)}
        token = jwt.encode(payload, self._secret, algorithm=self._algorithm)
        refresh_token = self.get_signature(token)
        return token, refresh_token

    def decode(self, token: str, expiration: bool = True) -> dict:
        return jwt.decode(
            token,
            self._secret,
            algorithms=[self._algorithm],
            options={"verify_exp": expiration},
        )
    ...
    def get_subject(self, token: str, expiration: bool = True) -> str:
        decoded = self.decode(token, expiration)
